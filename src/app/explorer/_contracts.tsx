...
> = {
  [CONTRACTS.STAKING]: {
    name: "Staking",
    description:
      "Lock EAST to raise your mining tier. Stake is currently locked-only — slashing is not implemented yet (see docs).",
  },
  [CONTRACTS.VESTING]: {
    name: "Vesting",
    description:
      "Founder allocation release schedule — 12-month cliff, then monthly for 36 more months.",
  },
  [CONTRACTS.MINING]: {
    name: "Mining",
    description:
      "Epoch-based mining rewards and Light Node participation bonus claims.",
  },
  [CONTRACTS.VALIDATOR]: {
    name: "Validator",
    description:
      "Recovery-round governance voting — approve/reject to help restore the network after a halt.",
  },
};
...
export default function ContractsContent() {
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        const data = await getContractCallStats();
        setStats(data);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="px-3 space-y-3">
      ...
      {Object.values(CONTRACTS).map((address) => {
        ...
        return (
          <Card
            key={address}
            className="bg-card/20 border-border/20"
          >
            <CardContent className="p-4 space-y-3">
              ...
              <div>
                <p className="text-[8px] text-white/30 uppercase font-black mb-1.5">
                  Callable Functions
                </p>
