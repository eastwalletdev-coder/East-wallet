...
export async function execute(
  ...
  const vestingRes = await identityClient.query(`
    SELECT * FROM identity.vesting ORDER BY created_at DESC LIMIT 1 FOR UPDATE
  `);
  if (!vestingRes.rows.length) return { success: false, error: 'NO_VESTING_SCHEDULE' };
